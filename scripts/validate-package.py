...
skill_version = require_match(
    re.search(r'(?m)^\s+version:\s*["\']?([0-9]+\.[0-9]+\.[0-9]+(?:\+[0-9A-Za-z.-]+)?)["\']?\s*$', yaml_metadata),
    "Add metadata.version to SKILL.md as a three-part version, optionally with a +local suffix",
).group(1)
readme_version = require_match(
    re.search(r"(?m)^- \*\*([0-9]+\.[0-9]+\.[0-9]+(?:\+[0-9A-Za-z.-]+)?)\*\*", README),
